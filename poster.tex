...
\title[]{{\fontsize{220}{240}\selectfont \vspace{2.5cm} \\
    Clustering and Diagnosing Patients with Rare Genetic Disorders
    \vspace{2.5cm} }} \author[]{\huge Jialin Song and Jonathan Zung}
    ...
    \begin{block}{\Large HPO}
      \begin{columns}[T]
        ...
        \begin{column}{.5\textwidth}
          ...
          standardized terms describing abnormal human phenotypes as a
          directed acyclic graph. More specific terms are children of
          more general terms. For example, ``dislocation'' is a child
          of ``abnormality of the joints''.
        \end{column}
      \end{columns}
    \end{block}
 ...
 \begin{column}{0.3\linewidth}
   \vbox to 0.82\textheight{
     ...
     \begin{block}{\Large Patient-Patient similarity metrics}
       ...
       standard clustering algorithm (in our case, spectral
       clustering).
       ...
       \item Information Metric: Here we count shared phenotypes
         weighted by rareness, so that patients sharing rarer
         phenotypes are more similar. The similarities are normalized
         by the analagous weighted count of the total number of phenotypes
         for the two patients.
       \end{itemize}
     \end{block}
     \begin{block}{\Large Mixture Models}
       Formulate a probabilistic model for patient phenotypes with a
       single latent variable $d$ representing the underlying
       disorder. Use the EM algorithm to fit the model, and then
       cluster by the inferred values of $d$.
       ...
       \item Conditional Mixture: Phenotypes are conditionally
         independent given $d$ and their common ancestors in HPO.
       \end{itemize}
     \end{block}
     \vfill
 ...
 \begin{column}{0.3\linewidth}
   \vbox to 0.82\textheight{
     ...
     \begin{block}{\Huge Clustering Results }
       \begin{figure}
         \centering
         \fbox{\includegraphics[width=0.6\textwidth]{cluster_comparison.png}}
         \vspace{0.5cm}
     ...
     \begin{block}{\Huge Diagnosis Results}
       \begin{figure}
         \centering
         \fbox{\includegraphics[width=.6\textwidth]{sim_patients}}
         \vspace{0.5cm}
